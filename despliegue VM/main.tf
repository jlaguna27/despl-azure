...
#Habilitar ingreso ssh con el usuario administrador sin clave, con la ruta de la llave
  admin_ssh_key {
...
#Distribucioń del sistema operativo linux ubuntu para las VM
...
#Se comenta por error en la suscripción, supera el limite de la suscripción, ya se creo el worker1
## Crear IP Pública para el Worker 2
 #resource "azurerm_public_ip" "var_Worker2" {
   #name                         = "publicIPWorker2"
   #location                     = azurerm_resource_group.var.location
   #resource_group_name          = azurerm_resource_group.var.name
   #allocation_method            = "Dynamic"
   #sku                          = "Basic"
...
#
#
## Crear interfaz de red para el Worker 2
#resource "azurerm_network_interface" "var_Worker2" {
   #name                = "NIC_Worker2"
   #location            = azurerm_resource_group.var.location
   #resource_group_name = azurerm_resource_group.var.name
#
   #ip_configuration {
     ...
     #subnet_id                     = azurerm_subnet.var.id
     #private_ip_address_allocation = "Dynamic"
     #public_ip_address_id          = azurerm_public_ip.var_Worker2.id
   #}
 #}
#
 ##Crear conjunto de disponibilidad para el Worker 2
#resource "azurerm_availability_set" "var_Worker2" {
   #name                         = "AvsWorker2"
   #location                     = azurerm_resource_group.var.location
   #resource_group_name          = azurerm_resource_group.var.name
   #platform_fault_domain_count  = 2
 ...
 #}
#
## Create virtual machine Worker 2
#resource "azurerm_linux_virtual_machine" "var_Worker2" {
  #name                  = "VMWorker2"
  #location              = azurerm_resource_group.var.location
  #availability_set_id   = azurerm_availability_set.var_Worker2.id
  #resource_group_name   = azurerm_resource_group.var.name
  #network_interface_ids = [azurerm_network_interface.var_Worker2.id]
  #size               = "Standard_E2s_v3"
  #admin_username        = "administrador"
  #disable_password_authentication = true
#
